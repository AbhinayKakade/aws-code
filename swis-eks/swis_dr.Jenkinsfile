pipeline { 
    ...
    stages { 
        ...
        //     steps { 
        //         git branch: '${BRANCH}', credentialsId: 'test-cred', url: 'git@bitbucket.org:experience-com/m2-swis.git'
        //         script {
        ...
        //                 -Dsonar.sources=. \
        //                 -Dsonar.css.node=. \
        //                 -Dsonar.host.url=http://172.20.39.188:9000"
        //                 }
        //         cleanWs()
        //     }
        //     }
        ...
        stage('Building Docker image') { 
            ...
            steps{
                ...
                script {  
                    ...
                    docker.build("swis:latest", "-f Docker/dr-m2-swis-Dockerfile .")
                    }
                }
                ...
                script
                {
                    sh '''aws eks update-kubeconfig --name "${CLUSTER_NAME}" --profile "${CLUSTER_PROFILE}" --region "${CLUSTER_REGION}"
                        ...
                        if kubectl get namespace "${SWIS_NAMESPACE}"; then
                            ...
                            envsubst < ./Kubernetes/deploy_dr.yaml   | kubectl rollout restart -f -
                        else
                            envsubst < ./Kubernetes/namespace.yaml  | kubectl apply -f -
                            kubectl create secret generic "${SWIS_SECRET}" --from-env-file=/var/lib/jenkins/Docker-Kubernetes/swis/swis-secret.env -n "${SWIS_NAMESPACE}"
                            envsubst < ./Kubernetes/swis_dr_deploy.yaml   | kubectl apply -f -
                        fi
                        '''
