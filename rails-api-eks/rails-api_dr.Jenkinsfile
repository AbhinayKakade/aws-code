            ...
            steps{
                ...
                script {  
                    ...
                    withCredentials([usernamePassword(credentialsId: 'EKS_DEVTEST_RAILS_SECRET_KEY_BASE', passwordVariable: 'RAILS_SECRET_KEY_BASE', usernameVariable: 'DEVTEST_RAILS_SECRET_KEY_BASE')]) {
                            docker.build("rails-api:latest", "--build-arg RAILS_ENV=${env.DEVTEST_RAILS_ENV} --build-arg SECRET_KEY_BASE=${RAILS_SECRET_KEY_BASE} --build-arg RAILS_MIGRATIONS=${RAILS_MIGRATIONS} --build-arg RAILS_SEED=${RAILS_SEED} -f Docker/Dockerfile.dr .")
                        }
                    }
